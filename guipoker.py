# guipoker.ipynb
#import python import_ipynb to import ipynb files
import import_ipynb
#import graphics package
from graphics import *
#use from to import ipynb files
from pokerapp import PokerApp
from button import Button
from cdieview import ColorDieView
#create the graphics interface class
class GraphicsInterface:
#define the init
    def __init__(self):
        self.win = GraphWin("Winners Circle", 600, 400)
        self.win.setBackground("gold")
        banner = Text(Point(300,30), "Super Duper Dice Poker !!")
        banner.setSize(24)
        banner.setFill("red3")
        banner.setStyle("bold")
        banner.draw(self.win)
        self.msg = Text(Point(300,380), "Welcome To The Winners Table")
        self.msg.setSize(18)
...
#define  the add dice buttons function
...
#define the set money function
    def setMoney(self, amt):
        self.money.setText("${0}".format(amt))
#define the show result function
    def showResult(self, msg, score):
        if score > 0:
            text = "{0}! You win ${1}".format(msg, score)
        else:
            text = "Result: {0}".format(msg)
        self.msg.setText(text)
#define the set dice function
    def setDice(self, values):
        for i in range(5):
            self.dice[i].setValue(values[i])
#define the want-to-play function
    def wantToPlay(self):
...
#define the close function
    def close(self):
        self.win.close()
#define the choose dice function
    def chooseDice(self):
